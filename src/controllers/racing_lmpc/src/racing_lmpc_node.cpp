...
{
RacingLMPCNode::RacingLMPCNode(const rclcpp::NodeOptions & options)
: rclcpp::Node("racing_lmpc_node", options),
  dt_(utils::declare_parameter<double>(this, "racing_lmpc_node.dt")),
  config_(lmpc::mpc::racing_lmpc::load_parameters(this)),
  track_(std::make_shared<RacingTrajectory>(
      utils::declare_parameter<std::string>(
        this, "racing_lmpc_node.race_track_file_path"))),
  model_(vehicle_model::vehicle_model_factory::load_vehicle_model(
      utils::declare_parameter<std::string>(
        this, "racing_lmpc_node.vehicle_model_name"), this)),
  mpc_(std::make_shared<RacingLMPC>(config_, model_)),
  ...
  f2g_(track_->frenet_to_global_function().map(mpc_->get_config().N))
{
  ...
  vehicle_state_sub_ = this->create_subscription<mpclab_msgs::msg::VehicleStateMsg>(
    "vehicle_state", 1, std::bind(
      &RacingLMPCNode::on_new_state, this,
      std::placeholders::_1), sub_options);

  if (config_->step_mode == RacingLMPCStepMode::CONTINUOUS) {
    // initialize the timers
    step_timer_ = this->create_wall_timer(
      std::chrono::duration<double>(dt_), std::bind(&RacingLMPCNode::on_step_timer, this));
  }
}

void RacingLMPCNode::on_new_state(const mpclab_msgs::msg::VehicleStateMsg::SharedPtr msg)
{
  state_msg_mutex_.lock();
  vehicle_state_msg_ = msg;
  state_msg_mutex_.unlock();
  if (config_->step_mode == RacingLMPCStepMode::STEP) {
    on_step_timer();
  }
}

void RacingLMPCNode::on_step_timer()
{
  using casadi::DM;
  using casadi::Slice;
  ...
  if (!mpc_->solved()) {
    ...
    } else if (config_->step_mode == RacingLMPCStepMode::STEP) {
      sol_in_["x_ic"] = x_ic;
    } else {
      throw std::runtime_error("Unknown RacingLMPCStepMode");
    }
...
int main(int argc, char * argv[])
{
  ...
  auto node = std::make_shared<lmpc::mpc::racing_lmpc::RacingLMPCNode>(options);
