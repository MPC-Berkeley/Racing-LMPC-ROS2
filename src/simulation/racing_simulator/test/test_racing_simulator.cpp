...
using lmpc::mpc::racing_simulator::RacingSimulator;
...
const auto share_dir = ament_index_cpp::get_package_share_directory("racing_simulator");
...
const auto trajectory_share_dir = ament_index_cpp::get_package_share_directory(
  "racing_trajectory");
RacingSimulator::SharedPtr get_mpc()
{
  ...
  auto test_node = rclcpp::Node("test_racing_simulator_node", options);
  ...
  auto config = lmpc::mpc::racing_simulator::load_parameters(&test_node);
  auto mpc = std::make_shared<RacingSimulator>(config, model);
...
TEST(RacingSimulatorTest, MPCSolveTest)
{
...
TEST(RacingSimulatorTest, MPCSolveTestStartDeviated)
{
  const lmpc::Pose2D x0_pose2d{
    85.4, -113.3, -2.3532
  };
...
// TEST(RacingSimulatorTest, MPCSolveTestAtJoint)
// {
//   const lmpc::Pose2D x0_pose2d{
//     -6.04, 5.76, -1.11
//   };
